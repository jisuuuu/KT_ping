# ICMP ping libs
# author : chidea
# code URL : https://gist.github.com/chidea/955cea841e5c76a7e5ee8aa02234409d
...
def do_one(dest_addr, timeout=1):
    ...
    try:
        ...
        print(f'[{e}] ip address : {dest_addr}')
...
def ping(dest_addr, timeout=2, count=4):
    ...
    for i in range(count):
        ...
        if delay is None:
            ...
            if min_resp > delay:
                min_resp = delay
            # print("ICMP Ping try[%s] IP:[%s] Response Time[%s]" % (i, dest_addr, delay))
    if count == count - suc:
        return [0, 0, 0, 0]
    else:
        return [suc, min_resp, max_resp, round(avg / suc, 2)]
